...
# Helper function to process file (shared between both endpoints)
async def _process_file_internal(file_path: str, task: str, chat_name: str, current_user: str):
    """Internal function to process a file - shared by both endpoints"""
    summary = "Sorry, we could not process your request. Please try again or contact support if the issue persists."
    ...
    for attempt in range(5):
        ...
        try:
            ...
            raise Exception(f"Dependency install failed: {e}")
...
# New endpoint for file uploads (for Streamlit frontend)
@router.post("/process")
async def process_file_upload(
    ...
    """Process an uploaded Office file with a natural language prompt"""
    try:
        # Validate file type
        file_ext = os.path.splitext(file.filename)[1].lower()
        if file_ext not in ['.docx', '.xlsx', '.pptx']:
            return JSONResponse(
                {"error": f"Unsupported file type: {file_ext}. Please upload .docx, .xlsx, or .pptx files."},
                status_code=status.HTTP_400_BAD_REQUEST
            )
        
        # Save uploaded file to temp directory
        uploaded_file_path = os.path.join(TEMP_DIR, f"uploaded_{uuid.uuid4()}_{file.filename}")
        with open(uploaded_file_path, "wb") as f:
            content = await file.read()
            f.write(content)
        
        print(f"[UPLOAD] Saved uploaded file to {uploaded_file_path}")
        
        # Use a default chat_name if not provided
        chat_name = f"upload_{uuid.uuid4().hex[:8]}"
        
        # Process the file using the shared internal function
        return await _process_file_internal(uploaded_file_path, prompt, chat_name, current_user)
        
    except Exception as e:
        print(f"[ERROR] Unexpected error in process_file_upload: {e}")
        return JSONResponse(
            {"error": f"Internal server error: {str(e)}"}, 
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR
        )

# Original endpoint for JSON requests (backward compatibility)
@router.post("/process/")
async def process_file(request: ProcessRequest, current_user: str = Depends(get_current_username)):
    """Process a file by file path (JSON request) - original endpoint for backward compatibility"""
    try:
        return await _process_file_internal(request.file_path, request.task, request.chat_name, current_user)
    except Exception as e:
